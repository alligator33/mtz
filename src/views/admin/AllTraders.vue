<template>
  <div class="body">
    <h1 v-if="message !== null" class="has-background-success py-2">
      {{ message }}
    </h1>
            ...
            <table class="table is-fullwidth">
              ...
              <template v-for="(trader, id) in traders">
                <tr :key="id">
                  ...
                  <td>
                    <span class="is-size-5">${{ trader.startingPrice }}</span>
                  </td>
                  <td>
                    <img
                      class="rating"
                      :src="`/img/icons/rating/${trader.rating}-star.png`"
                    />
                  </td>
                  <td>
                    <span class="is-capitalized">{{ trader.level }}</span>
                  </td>
                  ...
                  <td>
                    <div>
                      <router-link
                        :to="{
                          name: 'viewTrader',
                          params: { id: trader.code },
                        }"
                        class="button has-background-success is-pulled-left"
                        ><i class="far fa-eye"></i>
                      </router-link>

                      <button @click="modalVisibility" class="button is-danger">
                          ...
                          <div class="mx-5">
                            <div class="has-background-dark py-5 rounded-lg">
                              <div class="has-text-centered has-text-white">
                                <h1 class="">
                                  This action will delete this trader, all its
                                  users, comment and reply
                                </h1>
                                <p>Are you sure you want to do this ?</p>
                                <div class="my-4">
                                  <button
                                    @click="modalVisibility"
                                    class="button is-danger mx-5"
                                  >
                                    No
                                  </button>
                                  <button
                                    @click="deleteTrader(trader.code)"
                                    class="button is-success mx-5"
                                  >
                                    Yes
                                  </button>
                                </div>
...
export default {
  ...
  methods: {
    ...
    deleteTrader(code) {
     $axios.delete("/api/traders/" + code).then(() => {
    ...
    searchTrader() {
      $axios
        .get(`/api/traders/?search=${this.searchBox}`)
        .then(({ data }) => {
          this.traders = data;
          this.loaded = true;
        })
