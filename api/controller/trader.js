...
module.exports = {
  //get all traders
  /*   index: async (req, res) => {
    console.log("get all traders here oh ??s");
    ...
    if (req.query.hasOwnProperty("search")) {
      ...
      traders = await Trader.aggregate([
        {
          $match: {
            display: true,
          },
 ...
 */
  ...
  getAllEvery: async (req, res) => {
    let traders = await Trader.aggregate([
      {
        $match: {
          $or: [{ display: true }],
        },
  ...
  search: async (req, res) => {
    const traders = await Trader.find({
      name: { $regex: new RegExp(`.*hall.*`), $options: "i" },
    });
    res.status(200).json(traders);
  },
  ...
  replaceTrader: async (req, res) => {
    ...
    const result = await Trader.findOneAndUpdate(
      { code: req.value.params.code },
      newTrader
    );
  ...
  getTraderGuests: async (req, res) => {
    const trader = await Trader.findOne({ code: req.params.code }).populate({
      path: "guests",
      options: { sort: { createdAt: 1 } },
    });
    console.log(trader.guests, "here i am ");
